...
spatio-temporal variogram fitting, Kriging prediction, and
cross-validation for common R spatio-temporal data classes.
...
Before installing, make sure the core spatial dependencies are
available: `gstat`, `spacetime`, `sp`, `sf`, `sftime`, and `automap`.
The `cubble` package is suggested and is only required when working
directly with `cubble_df` objects.

## Main Features

- Automatic spatio-temporal variogram fitting with
  [`autofitVariogramST()`](https://sigmafelix.github.io/autoSTK/reference/autofitVariogramST.md).
- Automatic spatio-temporal Kriging with
  [`autoKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.md).
- Explicit fit/predict workflow with
...
- Cross-validation with
  [`autoKrigeST.cv()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.cv.md)
  using spatial, temporal, spatio-temporal, or random folds.
- Support for `STFDF`, `STSDF`, `STIDF`, `sftime`, and `cubble` inputs
  in the main fitting and prediction workflow.
- Conversion helper
  [`cubble_to_sftime()`](https://sigmafelix.github.io/autoSTK/reference/cubble_to_sftime.md)
  for nested or temporal `cubble` objects.
...
- [`autofitVariogramST()`](https://sigmafelix.github.io/autoSTK/reference/autofitVariogramST.md):
  automatically fits a spatio-temporal variogram.
- [`fitVariogramST()`](https://sigmafelix.github.io/autoSTK/reference/fitVariogramST.md):
  fits a variogram and returns a reusable `STVariogramFit` object.
- [`predictKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/predictKrigeST.md):
  predicts from a fitted `STVariogramFit` object.
- [`autoKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.md):
  fits the variogram and predicts in one call.
- [`autoKrigeST.cv()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.cv.md):
  cross-validates spatio-temporal Kriging results.
- [`cubble_to_sftime()`](https://sigmafelix.github.io/autoSTK/reference/cubble_to_sftime.md):
  converts `cubble_df` objects to `sftime`.

## Data Classes

The main Kriging workflow accepts both legacy `spacetime` classes and
modern simple-feature classes:
...
Use
[`autoKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.md)
when you have repeated measurements at monitoring stations and want
predictions on a future spatio-temporal grid. This is the classic
`STFDF`/`STSDF` workflow.
...
Use `sftime` when your data already lives in an `sf`-style workflow with
one geometry column and one time column.
[`autoKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.md)
accepts `sftime` for both training data and prediction data.
...
and
[`predictKrigeST()`](https://sigmafelix.github.io/autoSTK/reference/predictKrigeST.md)
are useful when you want to reuse the same fitted variogram across
several prediction grids.

### Use `cubble` Data

Use `cubble` when your workflow stores station metadata in the spatial
face and repeated measurements in the temporal face. `autoSTK` can
convert either the nested or temporal face to `sftime`.
...
You can also pass a `cubble_df` directly to the main Kriging workflow
when supplying a prediction grid:
...
object first, because
[`autoKrigeST.cv()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.cv.md)
builds folds from the `@sp` and `@time` slots.
...
[`autoKrigeST.cv()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.cv.md)
evaluates prediction performance by holding out different parts of the
...
Lower `RMSE` and `MAE` indicate smaller prediction errors. `BIAS` values
close to zero indicate little systematic over- or under-prediction.

For `fold_dim = "spacetime"`, `nfold` must be a perfect square, such as
`4`, `9`, or `16`, because the folds are built from spatial groups
crossed with temporal groups.

## Notes

- Projected coordinate reference systems are recommended because
  variogram distances are computed in map units.
- Universal Kriging formulas such as `PM10 ~ elevation + traffic`
  require `new_data` with matching covariate columns.
- `predict_chunk` can be used for large prediction grids to reduce
  memory pressure.
- `variogram_from_full = TRUE` in
  [`autoKrigeST.cv()`](https://sigmafelix.github.io/autoSTK/reference/autoKrigeST.cv.md)
  fits one variogram on the full data set and reuses it across folds,
  which can be much faster when that assumption is appropriate.
