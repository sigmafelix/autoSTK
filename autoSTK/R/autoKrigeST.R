...
autoKrigeST = function(formula, input_data, new_data, data_variogram = input_data, block = 0,
...
# This function performs an automatic Kriging on the data in input_data
{
    ...
    if(length(unique(input_data[[col_name]])) == 1) stop(sprintf("All data in attribute \'%s\' is identical and equal to %s\n   Can not interpolate this data", col_name, unique(input_data[[col_name]])[1]))
  	...
  	if(missing(new_data)) new_data = create_new_data.ST(input_data, mode = newdata_mode)
    ...
    variogram_object = autofitVariogramST(formula,
                      data_variogram,
    ...
    krige_result = krigeST(formula,
                      input_data,
                      new_data,
                      variogram_object$jointSTV,
                      #block = block,
					  ...)
