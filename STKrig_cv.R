...
## imputeKrig.cv_dep: skip calculation of whole dataset (*whole calculation for evaluation use)
## 2017-03-14: timefold, spacefold zero lag subset are to be retrieved
## 2017-03-14: Automatic C_s(0), C_t(0) fit
imputeKrig.cv_dep <- function(stf, pm = 'PM10',
  ...
  for (i in 1:nfold){
    ...
    lower.stvgm = extractPar(variost.mod) * 0.8
    upper.stvgm = extractPar(variost.mod) * 1.25
