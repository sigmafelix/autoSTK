...
autofit.vgmST <- function(stf, formula, typestv='sumMetric',
  ...
  stva <- plot.sti.set(stf=stf, formula = formula, 
                        tlags = tlags, cutoff = cutoff, width = width, 
                        wireframe=FALSE)
  stva.sp <- EmpVario_dist(stva, bound = cutoff)
  stva.ts <- EmpVario_dist(stva, spatial = FALSE)
  
  stva.sp.fit <- autofitVariogram_mid(formula=NULL, verbose=TRUE,
                                      input_vgm = stva.sp, 
                                      model = candidate_model)
  stva.ts.fit <- autofitVariogram_mid(formula=NULL, verbose=TRUE,
                                      input_vgm = stva.ts, 
                                      model = candidate_model)
  ...
  stv.ani <- estiStAni(stva, interval = c(0.25, 4)*median(stva$spacelag)*0.5, 
                       method = aniso_method, 
